...
void loadMesh(const aiMesh* aiMeshPtr, Mesh& newMesh, Scene& newScene) {
	...
	vec3 pos = newMesh.minv;
	for (uint32_t i = 0; i < newMesh.vertices.size(); i++)
		newMesh.vertices[i].position = newMesh.vertices[i].position - newMesh.minv;
	newMesh.minv = vec3(0.0);
	newMesh.maxv = newMesh.lenv;
	newMesh.setTranslation(pos);
...
void Scene::addToDynamicsWorld(btDiscreteDynamicsWorld* dw) {
	for (uint32_t i = 0; i < models.size(); i++)
		models[i].addToDynamicsWorld(dw);
}
