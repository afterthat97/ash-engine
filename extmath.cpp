...
void getDistanceOfLineLine(vec3 st1, vec3 dir1, vec3 st2, vec3 dir2, vec3& intersection1, vec3& intersection2, float& dis) {
	dir1 = normalize(dir1);
	dir2 = normalize(dir2);
	vec3 n = normalize(cross(dir1, dir2));
	if (dot(st2 - st1, n) < 0) n = -n;
	dis = dot(st2 - st1, n);

	vec3 u = st1 + n * dis - st2;
	float t = length(cross(dir2, u)) / length(cross(dir1, dir2));
	if (dot(cross(dir2, u), cross(dir1, dir2)) < 0) t = -t;

	intersection1 = st1 + dir1 * t;
	intersection2 = intersection1 + n * dis;
}
